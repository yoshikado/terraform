resource "terraform_data" "lxd_add_trust_certificate" {
  input = {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "lxc auth group permission add ${self.input.group_name} server ${self.input.entitlement}",
      "lxc auth identity create tls/${self.input.user_name} ${basename(self.input.client_cert_path)} -g ${self.input.group_name}",
      "rm ${basename(self.input.client_cert_path)}",
    ]
    when = create
  }
