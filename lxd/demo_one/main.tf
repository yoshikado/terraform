...
resource "lxd_network_forward" "forward_container1" {
  ...
  project        = lxd_project.project1.name
  listen_address = var.floatingip_container1
...
resource "lxd_network_forward" "forward_vm1" {
  network        = lxd_network.network_project2.name
  project        = lxd_project.project2.name
  listen_address = var.floatingip_vm1

  config = {
    target_address = lxd_instance.vm1.ipv4_address
  }
}